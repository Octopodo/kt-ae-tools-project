...
class __KT_ProjectImport {
    ...
    private _parseImportOptions(
        ...
        const importOptions: ImportOptions =
            options.importOptions || new ImportOptions();
        let importAs: ImportAsType = ImportAsType.FOOTAGE;
        if (options.importAs === "comp") {
    ...
    private _importFile(
        ...
        itemChecker: (item: _ItemClasses) => boolean = (item) => true
    ): _ItemClasses | Boolean {
        const importOptions = this._parseImportOptions(options);
        if (
            !importOptions ||
            !importOptions.canImportAs(importOptions.importAs)
        ) {
            return false;
        }
        try {
    ...
    file(options: KT_ImportOptions): _ItemClasses | Boolean {
        return this._importFile(options);
    }
    audio(options: KT_ImportOptions): _ItemClasses | Boolean {
        const imported = this._importFile(options, (file) =>
